...
// Macro for marking the beginning of a new namespace `NAME`
...
constexpr std::size_t hash_tuple_impl(const TUPLE& tuple)
{
    if constexpr (INDEX >= std::tuple_size<TUPLE>::value) {
        ...
        auto hasher = std::hash<std::tuple_element_t<INDEX, TUPLE>> {};
        return hash_combine(
            hasher(std::get<INDEX>(tuple)), hash_tuple_impl<TUPLE, INDEX + 1>(tuple)
        );
    }
}

/**
 * @brief Hash a tuple of arbitrary number of individually hashable elements
 *
 * @param tuple A tuple whose elements can be hashed individually
